/* Problem #2
Code Overview
Header Files and Namespace: The code begins by including necessary libraries and declaring the use of the standard namespace.
Reading User Input:
The ReadPositiveNumber function ensures the program only proceeds with a valid positive number by repeatedly prompting the user until such a number is entered.
Perfect Number Check:
The isPerfectNumber function checks if the given number is equal to the sum of its proper divisors (i.e., numbers less than the number that divide it evenly). If the sum equals the original number, it is a perfect number.
Displaying the Result:
The PrintResults function utilizes isPerfectNumber to determine if the number is perfect and then prints the appropriate message.
Program Execution:
The main function ties everything together by calling ReadPositiveNumber to get input from the user and then passing that input to PrintResults to display the result.
This detailed commentary should help you understand each part of the code and its purpose.
