...
ReadPositiveNumber prompts for a positive integer, ensuring valid input.
RandomNumber generates a random integer in a specified inclusive range.
Array Initialization & Operations:
FillArrayWith1toN fills the array with sequential numbers from 1 up to the number of elements.
ShuffleArray randomly swaps elements to shuffle the array using the Swap function.
Output:
PrintArray prints the elements of the array to the console.
Program Flow:
In main, the program seeds the random number generator, reads the desired array size from the user, fills the array sequentially, prints it, shuffles the array, and prints the shuffled result.
