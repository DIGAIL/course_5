...
/*Code Overview
Random Number Generation: The RandomNumber function generates a random integer in a given inclusive range, which is used to populate the array with random values.
Array Population & Output:
FillArrayWithRandomNumbers asks the user for the number of elements and fills the array with random integers between 1 and 100.
PrintArray prints the array's contents.
Array Element Addition:
AddArrayElement is a helper function that appends a new element to the end of an array and updates its length.
Selective Array Copying:
CopyOddNumbers scans the source array and uses AddArrayElement to copy only odd numbers into a destination array.
Program Flow:
In main, the program seeds the random number generator, fills an array with random numbers, copies only the odd numbers to a second array, and then prints both the original array and the array containing only odd numbers.
