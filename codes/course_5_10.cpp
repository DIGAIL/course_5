/*
Solution Source Code #10:
objectivec

Code Overview
Header Files and Namespace: The program includes <iostream> and <string> for input/output and string manipulation, respectively, and uses the standard namespace.
...
Reversing a Number:
The ReverseNumber function extracts each digit from the number (using modulo and division) and builds the reversed number.
Palindrome Check:
The IsPalindromeNumber function compares the original number with its reversed version to determine if it is a palindrome.
Program Execution:
The main function gets user input, checks if it is a palindrome, and prints an appropriate message based on the result.
*/
